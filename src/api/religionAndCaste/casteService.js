...
export const religionAndCastService = {
  ...
  addReligion: async ({ r_name, r_name_mar, created_by }) => {
    ...
    const result = await childQuery(`CALL AddReligion(?,?,?,?)`, [
      r_name,
      r_name_mar,
      created_by,
      created_at,
  ...
  getReligionList: async () => {
    const result = await childQuery(`CALL GetAllReligion()`);
  ...
  addCaste: async ({ religion_id, c_name, c_name_mar, created_by }) => {
    const created_at = getEpochTime();
    const result = await childQuery(`CALL AddCaste(?,?,?,?,?)`, [
      religion_id,
      c_name,
      c_name_mar,
      created_by,
  ...
  getCasteList: async () => {
    const result = await childQuery(`CALL GetAllCaste()`);
    const data = result?.[0] || [];
